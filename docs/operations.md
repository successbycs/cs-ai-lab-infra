...
The governed `startup_enable` T480 operation creates a Windows Scheduled Task named `CS AI Lab Start`. At the configured Windows user's sign-in, it starts Ubuntu WSL, waits for Docker, then runs `docker compose up -d n8n`; Compose starts the PostgreSQL dependency as well. The task does not expose any ports or start the optional Ollama profile. `startup_disable` removes it.
