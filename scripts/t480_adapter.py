...
OPERATIONS: dict[str, dict[str, Any]] = {
    ...
    "openworker_deploy": {
        ...
        "wsl_script": (
            ...
            "if [[ -e \"$deployment_root\" ]]; then\n"
            "  [[ -d \"$deployment_root/.git\" ]] || { printf 'Refusing OpenWorker deployment: target is not a Git checkout: %s\\n' \"$deployment_root\" >&2; exit 4; }\n"
            "  [[ \"$(git -C \"$deployment_root\" remote get-url origin)\" == \"$repository_url\" ]] || { printf 'Refusing OpenWorker deployment: origin differs.\\n' >&2; exit 4; }\n"
            "  git -C \"$deployment_root\" diff --quiet && git -C \"$deployment_root\" diff --cached --quiet || { printf 'Refusing OpenWorker deployment: checkout has local changes.\\n' >&2; exit 4; }\n"
            "else\n"
            "  mkdir -p /home/chris/projects\n"
            ...
            "  [[ ! -f .env ]] || { printf 'Refusing OpenWorker deployment: configured checkout is not the reviewed revision.\\n' >&2; exit 4; }\n"
