...
# Do not bind 9001 locally: it is Penpot's service port and can be claimed by
# a local container. The relay has its own loopback port.
state_dir="/home/chris/.config/cs-ai-lab"; port="${PENPOT_TUNNEL_PORT:-19001}"
...
if ! curl --fail --silent --max-time 2 "http://127.0.0.1:${port}/" >/dev/null; then
  ...
  nohup python3 "$infra_root/penpot/scripts/penpot_ssh_proxy.py" --target "$target" --port "$port" --max-clients 4 >"$log_file" 2>&1 &
  printf '%s\n' "$!" > "$pid_file"
fi
for _ in $(seq 1 20); do curl --fail --silent --max-time 8 "http://127.0.0.1:${port}/" >/dev/null && break; sleep 1; done
curl --fail --silent --max-time 10 "http://127.0.0.1:${port}/" >/dev/null
