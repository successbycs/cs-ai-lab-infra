...
4. Apply the reviewed health-dashboard migration to the existing T480 database. Start the restart-managed dashboard through Compose and the T480 startup path, then publish only the normalized Healthcheck result to PostgreSQL and render the dashboard from those tables. With explicit approval, use the fixed firewall operation to expose the dashboard—not n8n or PostgreSQL—to the private LAN on TCP 8080, restricted to the Windows Private firewall profile. Verify the page from the T16.
