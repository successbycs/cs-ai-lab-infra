...
3. Show the exact boot-time scheduled-task configuration and rollback (`startup_disable`) before approval. It must run at system boot as `NT AUTHORITY\\SYSTEM`, invoke only the fixed WSL startup action, wait for Docker, and start PostgreSQL plus n8n.
4. Correct the interim sign-in task first: retain a minimal WSL keepalive process after Compose starts so WSL does not immediately shut down Docker, PostgreSQL, and n8n when the initiating command exits. Verify n8n and PostgreSQL remain healthy for at least five minutes after the T16 session returns.
